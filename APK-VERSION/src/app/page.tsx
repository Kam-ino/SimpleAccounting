...
import { Plus, TrendingUp, TrendingDown, DollarSign, PiggyBank, Target, Edit, Trash2, BarChart3, PieChart, Settings, Wifi, WifiOff } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      const [transactionsData, budgetsData, savingsData] = await Promise.all([
        ClientAPI.getTransactions(5),
        ClientAPI.getBudgets(3),
        ClientAPI.getSavingsGoals(3)
      ])
  ...
  const handleDeleteBudget = async (budgetId: string) => {
    ...
    try {
      await ClientAPI.deleteBudget(budgetId)
      fetchDashboardData()
    } catch (error) {
  ...
  const handleDeleteSavingsGoal = async (goalId: string) => {
    ...
    try {
      await ClientAPI.deleteSavingsGoal(goalId)
      fetchDashboardData()
    } catch (error) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground">
              Track your income, expenses, and financial goals - Works completely offline!
            </p>
          </div>
        </div>
